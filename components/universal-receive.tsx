...
export function UniversalReceive() {
  return (
    <section
      ...
      className="relative overflow-hidden border-t border-[rgba(4,31,61,0.08)] px-5 sm:px-8 py-28 sm:py-36 scroll-mt-20"
    >
      ...
      <div className="max-w-6xl mx-auto">
        <Reveal>
          <div className="flex items-center gap-2">
            <Zap className="h-3.5 w-3.5 text-[#041f3d]" />
            <span className="eyebrow">UNIVERSAL RECEIVE</span>
          </div>
        </Reveal>

        <Reveal delay={80}>
          <h2 className="font-bold text-[clamp(2.25rem,5vw,3.5rem)] tracking-[-0.03em] text-[#041f3d] max-w-3xl mt-5">
            One handle. <span className="text-[rgba(4,31,61,0.55)]">Any chain.</span>
          </h2>
        </Reveal>

        <Reveal delay={160}>
          <p className="mt-6 max-w-2xl text-base sm:text-lg leading-relaxed text-[#041f3d]/75">
          ...
          {ITEMS.map((item, i) => {
            ...
            return (
              <Reveal key={item.title} delay={i * 80}>
                <div className="rounded-3xl border border-[rgba(4,31,61,0.08)] bg-white p-6 h-full">
                  <span className="flex h-9 w-9 items-center justify-center rounded-2xl border border-[rgba(4,31,61,0.08)] bg-[#F5F7FB]">
                    <Icon
                      className="h-4 w-4 text-[#041f3d]"
                      strokeWidth={2.25}
                    />
                  </span>
                  <h3 className="mt-5 text-base font-bold text-[#041f3d]">
                    {item.title}
                  </h3>
