...
export function ChainsStrip() {
  return (
    <section
      data-theme="dark"
      className="border-y border-[#1E293B] bg-[#0A0F0F] px-5 py-10 sm:px-8 sm:py-14"
    >
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-8 md:grid-cols-2 md:gap-12">
        <div>
          ...
          <Reveal delay={80}>
            <h2 className="mt-3 text-2xl font-bold tracking-[-0.02em] text-[#F1F5F9] sm:text-3xl">
              Send from <span className="text-[#94A3B8]">any chain.</span>
            </h2>
          </Reveal>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:grid-cols-5">
          {CHAINS.map((c, i) => (
            <Reveal key={c.name} delay={i * 70}>
              <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-[#1E293B] bg-[#111827] px-2 py-4">
