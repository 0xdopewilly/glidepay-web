...
export function GetStarted({ appUrl }: { appUrl: string }) {
  ...
  return (
    <section className="relative overflow-hidden border-t border-[rgba(31,41,55,0.08)] bg-[#FFFFFF] px-5 py-28 sm:px-8 sm:py-36">
      ...
      <div className="mx-auto grid max-w-6xl gap-14 md:grid-cols-2">
        <div>
          <Reveal>
            <h2 className="text-[clamp(2.5rem,5vw,4.5rem)] font-bold leading-[1.02] tracking-[-0.03em] text-[#1F2937]">
              How to
              <br />
              <span className="text-[#8B5CF6]">Get Started</span>
            </h2>
          </Reveal>
          <Reveal delay={80}>
            <p className="mt-6 max-w-md text-base leading-relaxed text-[#1F2937]/75">
            ...
            return (
              <Reveal key={card.title} delay={i * 80}>
                <Link
                  href={card.href}
                  {...linkProps}
                  className="group glow-green-soft flex items-center justify-between gap-6 rounded-3xl border border-[rgba(31,41,55,0.18)] bg-[#060B1C] p-7 transition-all duration-300 [transition-timing-function:var(--ease-smooth)] hover:-translate-y-0.5 hover:[box-shadow:0_0_0_1px_rgba(139,92,246,0.25),0_24px_70px_-20px_rgba(139,92,246,0.45),0_0_90px_-20px_rgba(139,92,246,0.3)]"
                >
                  <div>
